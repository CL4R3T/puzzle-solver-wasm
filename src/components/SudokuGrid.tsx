...
export function SudokuGrid({
  ...
  useLayoutEffect(() => {
    ...
    if (c00 && c01) {
      pathThresholdRef.current = (c01.cx - c00.cx) * 0.5;
    }
  }, [cellCenters]);
  ...
  const { overlays, pathLines } = useMemo(() => {
    ...
    const result: CellOverlay[][] = Array(size)
      .fill(null)
      .map(() =>
        Array(size)
          .fill(null)
    ...
    const regionConstraints = constraints.filter(
      (c) => getCategoryForType(c.constraintType) !== "path",
    );
    const pathConstraints = constraints.filter(
      (c) => getCategoryForType(c.constraintType) === "path",
    );
  ...
  const tryPropagatePath = useCallback(
